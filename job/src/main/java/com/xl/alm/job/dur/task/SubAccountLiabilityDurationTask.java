...
@Slf4j
@Component
public class SubAccountLiabilityDurationTask {
    ...
    public boolean execute(String accountPeriod) {
        log.info("开始执行分账户负债久期汇总任务，账期：{}", accountPeriod);
        ...
        try {
            ...
            log.info("分账户负债久期汇总任务执行完成，账期：{}，耗时：{}ms，结果：{}", 
                    accountPeriod, (endTime - startTime), result);
            
            return result;
        } catch (Exception e) {
            long endTime = System.currentTimeMillis();
            log.error("分账户负债久期汇总任务执行失败，账期：{}，耗时：{}ms", 
