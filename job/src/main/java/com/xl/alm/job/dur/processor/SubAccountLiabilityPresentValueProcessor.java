...
@Slf4j
@Component
public class SubAccountLiabilityPresentValueProcessor implements BasicProcessor {
    ...
    public ProcessResult process(TaskContext taskContext) throws Exception {
        log.info("分账户负债现金流现值汇总处理器开始执行，任务参数：{}", taskContext.getJobParams());
        ...
        try {
            ...
            // 执行任务
            boolean result = subAccountLiabilityPresentValueTask.execute(accountPeriod);
            
            if (result) {
                return new ProcessResult(true, "分账户负债现金流现值汇总成功");
            } else {
                return new ProcessResult(false, "分账户负债现金流现值汇总失败");
            }
        } catch (Exception e) {
            log.error("分账户负债现金流现值汇总处理器执行异常", e);
            return new ProcessResult(false, "分账户负债现金流现值汇总处理器执行异常：" + e.getMessage());
        }
    }
