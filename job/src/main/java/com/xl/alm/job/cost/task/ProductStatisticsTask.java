...
public class ProductStatisticsTask {
    ...
    public boolean execute(String accountingPeriod) {
        log.info("开始执行分产品统计计算任务，账期：{}", accountingPeriod);
        long startTime = System.currentTimeMillis();
        
        try {
            boolean result = productStatisticsService.calculateProductStatistics(accountingPeriod);
            
            long endTime = System.currentTimeMillis();
            log.info("分产品统计计算任务执行完成，账期：{}，耗时：{}ms，结果：{}", 
                    accountingPeriod, (endTime - startTime), result);
            
            return result;
        } catch (Exception e) {
            long endTime = System.currentTimeMillis();
            log.error("分产品统计计算任务执行失败，账期：{}，耗时：{}ms", 
