...
@Slf4j
@Component
public class AccountEffectiveRateTask {
    ...
    public boolean execute(String accountingPeriod) {
        log.info("开始执行分账户有效成本率计算任务，账期：{}", accountingPeriod);
        ...
        try {
            ...
            log.info("分账户有效成本率计算任务执行完成，账期：{}，耗时：{}ms，结果：{}", 
                    accountingPeriod, (endTime - startTime), result);
            
            return result;
        } catch (Exception e) {
            long endTime = System.currentTimeMillis();
            log.error("分账户有效成本率计算任务执行失败，账期：{}，耗时：{}ms", 
