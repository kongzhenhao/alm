...
| -------- |---------| ---------- | -------- |
| MD0001 | 成本管理    | base_data_management | bdm |

#### 2.2.1 模块说明

**成本管理(MD0001)**：
- 产品属性表(t_base_product_attribute)：管理产品基本信息和保证成本率
- 万能平均结算利率表(t_base_universal_avg_settlement_rate)：管理万能险产品的结算利率
- 分红方案表(t_base_dividend_fund_cost_rate)：管理分红险产品的分红比例和投资收益率
- 法定准备金明细表(t_base_statutory_reserve_detail)：管理法定准备金数据
- 会计准备金明细表(t_base_accounting_reserve_detail)：管理会计准备金数据
- 法定准备金预测表(t_base_statutory_reserve_forecast)：管理准备金预测数据
- 分产品统计表(t_cost_product_statistics)：按产品汇总统计数据
- 分业务类型汇总表(t_cost_business_type_summary)：按业务类型汇总数据
- 分账户汇总表(t_cost_account_summary)：按账户汇总数据
- 分产品有效成本率表(t_cost_product_effective_rate)：计算各产品的有效成本率
- 分账户有效成本率表(t_cost_account_effective_rate)：计算各账户的有效成本率
...
| ---- | ------ |---------------------------------| ------ | ------ |
| TB0001 | 产品属性表 | t_base_product_attribute        | MD0001 | 基础产品信息 |
| TB0002 | 万能平均结算利率表 | t_base_universal_avg_settlement_rate | MD0001 | 万能险结算利率 |
| TB0003 | 分红方案表 | t_base_dividend_fund_cost_rate  | MD0001 | 分红险分红方案 |
| TB0004 | 法定准备金明细表 | t_base_statutory_reserve_detail | MD0001 | 法定准备金数据 |
| TB0005 | 会计准备金明细表 | t_base_accounting_reserve_detail | MD0001 | 会计准备金数据 |
| TB0006 | 法定准备金预测表 | t_base_statutory_reserve_forecast | MD0001 | 准备金预测数据 |
| TB0007 | 分产品统计表 | t_cost_product_statistics       | MD0001 | 产品统计数据 |
| TB0008 | 分业务类型汇总表 | t_cost_business_type_summary    | MD0001 | 业务类型汇总 |
| TB0009 | 分账户汇总表 | t_cost_account_summary          | MD0001 | 账户汇总数据 |
| TB0010 | 分产品有效成本率表 | t_cost_product_effective_rate   | MD0001 | 存储各产品的有效成本率计算结果 |
| TB0011 | 分账户有效成本率表 | t_cost_account_effective_rate   | MD0001 | 存储按账户汇总的有效成本率 |
| TB0012 | 负债现金流表 | t_dur_liability_cash_flow       | MD0001 | 负债现金流明细 |

#### 2.3.3 表集

##### 2.3.3.1 产品属性表(TB0001)

**表英文名**：t_base_product_attribute
**表中文名**：产品属性表
**所属模块**：基础数据管理(MD0001)
...
**表英文名**：t_base_universal_avg_settlement_rate
**表中文名**：万能平均结算利率表
**所属模块**：基础数据管理(MD0001)
...
**表英文名**：t_base_dividend_fund_cost_rate
**表中文名**：分红方案表
**所属模块**：基础数据管理(MD0001)
...
**表英文名**：t_base_statutory_reserve_detail
**表中文名**：法定准备金明细表
**所属模块**：基础数据管理(MD0001)
...
**表英文名**：t_base_accounting_reserve_detail
**表中文名**：会计准备金明细表
**所属模块**：基础数据管理(MD0001)
...
**表英文名**：t_base_statutory_reserve_forecast
**表中文名**：法定准备金预测表
**所属模块**：基础数据管理(MD0001)
...
**表英文名**：t_cost_product_statistics
**表中文名**：分产品统计表
**所属模块**：负债成本率计算(MD0001)
...
**表英文名**：t_cost_business_type_summary
**表中文名**：分业务类型汇总表
**所属模块**：负债成本率计算(MD0001)
...
| 用例编号   | 用例名称             | 用例描述 | 模块编号 |
|--------|------------------| - | ---- |
| UC0001 | 导入产品属性数据        | 导入产品属性数据，写入TB0001表 | MD0001 |
| UC0002 | 导入万能平均结算利率数据         | 导入万能平均结算利率数据，写入TB0002表 | MD0001 |
| UC0003 | 导入分红方案数据         | 导入分红方案数据，写入TB0003表 | MD0001 |
| UC0004 | 导入法定准备金明细数据      | 导入法定准备金明细数据，写入TB0004表 | MD0001 |
| UC0005 | 导入会计准备金明细数据     | 导入会计准备金明细数据，写入TB0005表 | MD0001 |
| UC0006 | 导入法定准备金预测数据        | 导入法定准备金预测数据，写入TB0006表 | MD0001 |
| UC0007 | 导入负债现金流数据        | 导入负债现金流数据，写入TB0012表 | MD0001 |
| UC0008 | 计算分产品统计数据        | 计算分产品统计数据，写入TB0007表 | MD0001 |
| UC0009 | 计算分业务类型汇总数据      | 计算分业务类型汇总数据，写入TB0008表 | MD0001 |
| UC0010 | 计算分账户汇总数据      | 计算分账户汇总数据，写入TB0009表 | MD0001 |
| UC0011 | 计算分产品有效成本率     | 计算分产品有效成本率，写入TB0010表 | MD0001 |
| UC0012 | 计算分账户有效成本率     | 计算分账户有效成本率，写入TB0011表 | MD0001 |

### 2.5 接口清单

| 接口编号   | 接口名称        | 接口描述 | 模块编号 |
| ------ |-------------| ---- | ---- |
| IF0001 | 导入产品属性数据   | 导入产品属性数据，写入TB0001表 | MD0001 |
| IF0002 | 导入万能平均结算利率数据    | 导入万能平均结算利率数据，写入TB0002表 | MD0001 |
| IF0003 | 导入分红方案数据    | 导入分红方案数据，写入TB0003表 | MD0001 |
| IF0004 | 导入法定准备金明细数据 | 导入法定准备金明细数据，写入TB0004表 | MD0001 |
| IF0005 | 导入会计准备金明细数据 | 导入会计准备金明细数据，写入TB0005表 | MD0001 |
| IF0006 | 导入法定准备金预测数据 | 导入法定准备金预测数据，写入TB0006表 | MD0001 |
| IF0007 | 导入负债现金流数据 | 导入负债现金流数据，写入TB0012表 | MD0001 |
...
(1) 遍历产品属性表中的产品记录，对每条记录执行以下操作：
   ...
   b. 根据精算代码从负债现金流HashMap中获取现金流数据
   c. 构建现金流值集(json)：
    根据精算代码从会计准备金明细HashMap中获取账面价值数据：
      - 第0个元素为账面价值（负值） 账面价值(value) = 账户价值 + 最优估计 + 风险边际 + 剩余边际 + 持续奖准备金 + 零头月红利
      - 第1-1272个元素为业务净现金流 账面价值(value) =（流出现金流*(-1) + 流入现金流）
   d. 计算有效成本率：
