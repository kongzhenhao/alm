...
import { tansParams, blobValidate } from '@/utils/lightning';
    ...
    if (
      ...
      if (!config.headers.hasChangedUrl) {
        config.url = config.url.replace(
          '',
          getUrlByTenantId(getUrlParams(window.location.href).tenantId) + '/'
        );
      }
      delete config.headers.hasChangedUrl;
    }
    return config;
  },
  (error) => {
...
// 通用下载方法
export function download(url, params, filename, need = true) {
