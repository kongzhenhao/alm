    ...
    >
      ...
      <el-table-column label="流程分类" align="center" prop="category">
        <template slot-scope="scope">
          {{ categorysToLabel(scope.row.category) }}
        </template>
      </el-table-column>
      <el-table-column
        label="流程名称"
...
export default {
  ...
  data() {
    return {
      ...
      queryParams: {
        ...
        category: null,
        key: null,
        tenantId: this.$route.query.isAll ? null : this.$route.query.tenantId,
        deployTime: null,
        derivedFrom: null,
  ...
  methods: {
    ...
    handleLoadXml(row) {
      this.$router.push({
        path: '/task/flowable/definition/model',
        query: row.deploymentId
          ? { deployId: row.deploymentId, tenantId: row.category }
          : {},
      });
    },
    /** 流程图查看 */
    ...
    handleUpdateSuspensionState(row) {
      ...
      updateState(params, row.category).then((res) => {
        this.$modal.msgSuccess(res.msg);
        this.getList();
      });
    },
