<template>
  <div class="app-container">
    <el-card class="box-card">
      ...
      <!--流程处理表单模块-->
      <el-col
        :span="taskTmpType.includes('_custom') ? 24 : 16"
        :offset="taskTmpType.includes('_custom') ? 0 : 6"
        v-if="variableOpen"
      >
        <div v-if="taskTmpType === 'normal'">
          <parser :key="new Date().getTime()" :form-conf="variablesData" />
        </div>
        <template v-else-if="taskTmpType.includes('_custom')">
          <!-- 自定义组件 -->
        </template>
        <div
          style="margin-left: 40%; margin-bottom: 20px; font-size: 14px"
          v-if="finished === 'true'"
        >
          <el-button
        ...
        <div class="block">
          <el-timeline>
            <el-timeline-item
              ...
              :key="index"
              :icon="setIcon(item.approvalStatus)"
              :color="setColor(item.approvalStatus)"
            >
    ...
    processVariables(taskId) {
      if (taskId) {
        ...
        getProcessVariables(taskId).then((res) => {
          ...
          let { oldData, data, dataDict } = variables;
          delete variables.oldData;
          delete variables.data;
          delete variables.dataDict;
          if (
            variables.fields.length === 1 &&
            variables.fields[0].__vModel__.includes('_custom')
          ) {
            this.taskTmpType = variables.fields[0].__vModel__;
            this.variablesData = {
              data: data,
              oldData: oldData,
              dataDict,
            ...
            return;
            // if (variables.fields[0].__vModel__ === "2") {
            //   this.taskTmpType = "2";
            //   this.variableOpen = true;
            //   return
            // }
          }
          variables.fields.forEach((item) => {
            item.__config__.defaultValue = dataDict[item.__vModel__];
            if (!oldData || !oldData.hasOwnProperty(item.__vModel__)) {
              item.__config__.changeType = 'add';
            } else {
              if (
                !(!dataDict[item.__vModel__] && !oldData[item.__vModel__]) &&
                dataDict[item.__vModel__] !== oldData[item.__vModel__]
              ) {
                item.__config__.changeType = 'update';
                item.__config__.oldData = oldData[item.__vModel__];
              }
            }
          });
    ...
    removeDuplicateObj(arr) {
      ...
      if (arr.length > 0) {
        arr = arr.reduce((newArr, next) => {
          obj[next.additionName]
            ? ''
            : (obj[next.additionName] = true && newArr.push(next));
          return newArr;
        }, []);
      } else {
        arr = [];
