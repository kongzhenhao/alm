  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      ...
      <el-form-item label="长短期标识" prop="termType">
        <el-select v-model="queryParams.termType" placeholder="请选择长短期标识" clearable>
          <el-option label="长期" value="L" />
          <el-option label="短期" value="S" />
        </el-select>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['cost:product:statistics:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['cost:product:statistics:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['cost:product:statistics:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['cost:product:statistics:export']"
        >导出</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleImport"
          v-hasPermi="['cost:product:statistics:import']"
        >导入</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="statisticsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="长短期标识" align="center" prop="termType" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.termType === 'L' ? '长期' : '短期' }}</span>
        </template>
      </el-table-column>
...
import {
  ...
  delProductStatistics,
  exportProductStatistics,
  importProductStatisticsTemplate,
  importProductStatistics
} from "@/api/cost/productStatistics";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/cost/product/statistics/importData"
      }
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listProductStatistics(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getProductStatistics(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateProductStatistics(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addProductStatistics(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除分产品统计编号为"' + ids + '"的数据项？').then(function() {
        return delProductStatistics(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.$modal.confirm('是否确认导出所有分产品统计数据项？').then(() => {
        this.exportLoading = true;
        return exportProductStatistics(this.queryParams);
      }).then(response => {
        this.$download.excel(response, '分产品统计数据.xlsx');
    ...
    /** 下载模板操作 */
    importTemplate() {
      importProductStatisticsTemplate().then(response => {
        this.$download.excel(response, '分产品统计数据模板.xlsx');
      });
    },
    // 文件上传中处理
